...
func (r *HostReconciler) ProcessLockHost(host_instance *starlingxv1.Host, host_obj *hosts.Host) error {
	mon_err, req_success := r.LockHostRequest(host_instance, host_instance, host_obj.ID, host_obj.Personality, "oam platform network")
	if !req_success {
	...
	if stb_host_instance.Name == instance.Name && r.LockUnlockAnnotationExists(instance) {
		...
		if err == nil {
			...
			if strategy_status.State != "applied" {
				return common.NewResourceStatusDependency("waiting for standby controller strategy to finish.")
			}
...
func (r *HostReconciler) LockHostRequest(notify_instance *starlingxv1.Host, host_instance *starlingxv1.Host, host_id string, host_personality string, reconcile_object string) (error, bool) {
